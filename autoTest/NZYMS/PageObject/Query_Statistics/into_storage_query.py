...
class Into_Storage_Query(BasePage):
    ...
    def search_and_check(self,input,number):
        ...
        check.equal(tableCheck1.get_value("入库位置"), input['入库位置1'] + input['入库位置2'])
        check.equal(tableCheck1.get_value("入库件数"), input['件数'])
        check.equal(tableCheck1.get_value("入库体积"), input['体积'])
        check.equal(tableCheck1.get_value("入库重量(kg)"), input['重量(kg)'])
