...
class Immediate_plan(BasePage):
    ...
    def Add_Plan(self, input):
        ...
        textInput.select_by_label('靠泊泊位', input["靠泊泊位"])
    ...
    def check_alongside_info(self,input):
        ...
        check.equal(self.get_text_value("计划起始尺码"), '0')
        check.equal(self.get_text_value("计划终止尺码"), "125")
        ...
        check.equal(self.get_text_value("实际起始尺码"), '0')
        check.equal(self.get_text_value("实际终止尺码"), "170")
