    ...
    def compute(self, w: str, up_type: str):
        if w == "wsw":
            if up_type == "piece":
                ...
                complemenHour = Decimal(str(self.gtpks)) * Decimal(str(self.woker_complemen)) / Decimal(
                    str(self.hatch_hour))
            elif up_type == "weight":
                normHour = Decimal(str(self.gtwg)) * Decimal(str(self.time_quota))
                complemenHour = Decimal(str(self.gtwg)) * Decimal(str(self.woker_complemen)) / Decimal(
                    str(self.hatch_hour))
            elif up_type == "hour":
            ...
            realHour = normHour * Decimal(str(self.adjust_factor)) * Decimal(str(self.factor)) * Decimal(
                str(self.percentage)) \
    ...
    def compare(self, w: str, up_type: str):
        try:
            ...
            out = "" if set1 == set2 else (set1.read(), set2.read(),
                                           "请求参数: 件数 " + str(self.gtpks) + ",吨位 " + str(self.gtwg) + ",工作时间 " + str(
                                               self.work_hour)
                                           + ",加人 " + str(self.woker_complemen) + ",计时工人数 " + str(
                                               self.woker_num) + ",调节系数 " + str(self.adjust_factor)
                                           + ",系数 " + str(self.factor) + ",加班率 " + str(
                                               self.overtime_rate) + ",加减工时 " + str(self.plus_minus_hour)
                                           + ",百分比 " + str(self.percentage) + ",工时定额 " + str(
                                               self.time_quota) + ",舱时量 " + str(self.hatch_hour))
