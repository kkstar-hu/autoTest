...
class DataRes(GetPg):
    ...
    def daynightworkhour_day(self, work_date:str):
        sql = self.load_sql(os.path.join(os.path.dirname(__file__),r"sql\daynightworkhour_day.sql")).format(workdate=work_date)
        res = self.select_from_table(sql)
    ...
    def daynightworksta_day(self, work_date:str):
        sql = self.load_sql(os.path.join(os.path.dirname(__file__),r"sql\daynightworksta_day.sql")).format(workdate=work_date)
        res = self.select_from_table(sql)
        self.logger.info('昼夜作业情况统计表-分货类结存(daynightworksta)-sql结果\n参数: work_date = {}\n'.format(work_date) + self.to_json(res))


    def daynightworksta_store(self, work_date:str):
        sql = self.load_sql(os.path.join(os.path.dirname(__file__),r"sql\daynightworksta_store.sql")).format(workdate=work_date)
        res = self.select_from_table(sql)
        self.logger.info('昼夜作业情况统计表-堆存情况(daynightworksta)-sql结果\n参数: work_date = {}\n'.format(work_date) + self.to_json(res))


    def daynightworksta_zj(self, work_date:str):
        sql = self.load_sql(os.path.join(os.path.dirname(__file__),r"sql\daynightworksta_zj.sql")).format(workdate=work_date)
        res = self.select_from_table(sql)
        self.logger.info('昼夜作业情况统计表-桩脚(daynightworksta)-sql结果\n参数: work_date = {}\n'.format(work_date) + self.to_json(res))


    def daynightworksta_paper(self, work_date:str):
        sql = self.load_sql(os.path.join(os.path.dirname(__file__),r"sql\daynightworksta_paper.sql")).format(workdate=work_date)
        res = self.select_from_table(sql)
        self.logger.info('昼夜作业情况统计表-本日纸浆提货情况(daynightworksta)-sql结果\n参数: work_date = {}\n'.format(work_date) + self.to_json(res))


    def daynightworksta_nin(self, work_date:str):
        sql = self.load_sql(os.path.join(os.path.dirname(__file__),r"sql\daynightworksta_nin.sql")).format(workdate=work_date)
        res = self.select_from_table(sql)
        self.logger.info('昼夜作业情况统计表-本日内贸进场情况(daynightworksta)-sql结果\n参数: work_date = {}\n'.format(work_date) + self.to_json(res))
