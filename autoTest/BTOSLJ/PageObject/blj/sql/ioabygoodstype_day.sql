...
with finaldata as(
	...
	today as(
		with res as(
			...
			(case when goa.goa_iofg='1' then '进' when goa.goa_iofg='2' then '出' end) as 进出标志, goa_opdate,
			...
			group by 大货类 , goa.goa_iofg, goa_opdate, ybk.ybk_name, trade_type
		)
		select pck2.pck_kind_name as 大货类, res.场地区域, DATE_FORMAT(goa_opdate,'%Y-%m-%d') as 日期, res.trade_type as 贸易类型,
	...
	select today.大货类, today.场地区域, today.日期, today.贸易类型, today.进出标志, today.进出件数, today.进出重量,
		nowstore.库场件数, nowstore.库场重量
	from today
	left join nowstore on nowstore.大货类=today.大货类 and nowstore.场地区域=today.场地区域 and nowstore.贸易类型=today.贸易类型
	union all
	select today.大货类, '全部' as 场地区域, today.日期, today.贸易类型, today.进出标志, SUM(today.进出件数), SUM(today.进出重量),
		SUM(nowstore.库场件数), SUM(nowstore.库场重量)
	from today
	left join nowstore on nowstore.大货类=today.大货类 and nowstore.场地区域=today.场地区域 and nowstore.贸易类型=today.贸易类型
	group by today.日期,today.大货类, today.贸易类型, today.进出标志
)
select *
from finaldata
where finaldata.日期 = '{workdate}' and finaldata.场地区域 = '{ybkname}'
