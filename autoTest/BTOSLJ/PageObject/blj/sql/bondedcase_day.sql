-- 保税仓库库场情况
with res as(
	select bod_no as 项号,CONCAT(vsl.vsl_cnname, '/', voy.voy_voyage) as 船名航次, bil.bil_bill_nbr as 提单号, bod.bod_gname as 货名,
		bil.bil_gtpks as 总件数, bil.bil_gtwg as 总重量, SUM(bod.bod_ntwg) as 总净重,
	...
	group by bod_id , bod_no, CONCAT(vsl.vsl_cnname, '/', voy.voy_voyage), bil.bil_bill_nbr, bod.bod_gname,
		bil.bil_gtpks, bil.bil_gtwg
)
select res.项号, res.船名航次,res.提单号,res.货名,res.总件数,res.总重量,res.总净重,res.进库件数,res.进库重量,
