-- 保税仓库进出报表
with res as(
	select bod_no as 料号, bod_record_no as 序号, CONCAT(vsl.vsl_cnname, '/', voy.voy_voyage) as 船名航次, bil.bil_bill_nbr as 提单号, bod.bod_gname as 货名,
		bil.bil_gtpks as 总件数, bil.bil_gtwg as 总重量, SUM(bod.bod_ntwg) as 总净重, ygc.ygc_no as 货位,
	...
	group by bod_id , bod_no, CONCAT(vsl.vsl_cnname, '/', voy.voy_voyage), bil.bil_bill_nbr, bod.bod_gname, bil.bil_gtpks,
		bil.bil_gtwg, ygc.ygc_no, DATE_FORMAT(bod_stock_date,'%Y-%m-%d'), bod_record_no
)
select res.料号,res.序号, res.船名航次,res.提单号,res.货名,res.总件数,res.总重量,res.总净重,res.货位,res.进库件数,res.进库重量,
...
from res
where res.进库日期 >= '{startdate}' and res.进库日期 <= '{enddate}';