...
with finaldata as(
	with res as(
		...
		from(
			...
			where bil.bil_bill_iefg = 'I' and bil.bil_bill_trade = 'W' and vsl.tenant_id = 'SIPGLJ'
	...
	select * from res
	union all
	select '合计' as 船名, '合计' as 航次, SUM(res.舱单件数), SUM(res.舱单重量), SUM(res.车直提件数), SUM(res.车直提重量),
