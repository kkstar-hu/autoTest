...
class CheckInBox(BasePage):
    ...
    def input_info(self, input):
        ...
        self.click("x",
                   "//div[starts-with(@class,'el-select-dropdown el-popper') and not (contains(@style,'display: "
                   "none'))]//span[contains(text(),'自卸架')]")
        self.input("x", "//span[text()='车架号：']//following-sibling::div/input", input['车架号'])
        if self.elementExist("x",
                             "//button[@class='el-button el-tooltip el-button--warning el-button--mini is-circle']"):
            pass
        else:
            self.click("x", "//span[text()='进口道口号：']//following-sibling::div//input")
            self.click("x",
                       f"//div[starts-with(@class,'el-select-dropdown el-popper') and not (contains(@style,'display: "
                       f"none'))]//span[contains(text(),{input['进口道口号']})]")
