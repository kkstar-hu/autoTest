...
class Gtos_text(BasePage):
    ...
    def input_by_number(self, label, value, index=1):
        ...
        """
        try:
            self.input_by_index("xpath", f"//label[contains(text(),'{label}')]//following-sibling::div//input", value,
                                index)
        except NoSuchElementException:
            self.logger.error(f"定位不到单行文本控件标签名:{label}")
            raise Exception("定位不到元素")
    ...
    def get_text_value(self, label, index=1):
        try:
            return self.get_text_index("xpath", f"//label[contains(text(),'{label}')]//following-sibling::div//input",
                                       index)
        except NoSuchElementException:
